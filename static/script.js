...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Disable download feature on Vercel
    if (isVercel) {
        btnDownload.disabled = true;
        btnDownload.style.opacity = '0.5';
        btnDownload.style.cursor = 'not-allowed';
        btnDownload.title = 'Download feature is only available in the local environment';
        showError('error-message', '⚠️ Downloads not available on Vercel. This feature requires spotdl + ffmpeg (long-running processes). Run locally for full features.');
    }
    ...
    convertBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Handle Vercel/Server errors that return HTML instead of JSON
            const contentType = response.headers.get("content-type");
            if (!contentType || !contentType.includes("application/json")) {
                const text = await response.text();
                throw new Error("Translation Service Unavailable: Please ensure the backend is running correctly.");
            }
